...
void FreeDocTable(DocTable table) {
  ...
  // STEP 1.
  // Free the two HashTables first and then free the table
...
DocID_t DTRegisterDocumentName(DocTable table, char *docname) {
  ...
  // STEP 2.
  // Using the hash values generated by FNV64 on the docname
  // to check if the document already exists
  int retval;  // Grab return values to check for errors
  res = FNVHash64((unsigned char *) docname, strlen(docname));
  retval = LookupHashTable(table->docname_to_docid, res, &oldkv);
  // Halt program if an error occurs
  Verify333(retval != -1);
  // Found that the document already exists
  if (retval == 1) {
    free(doccopy);
    free(docid);
