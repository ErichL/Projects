...
char *ReadFile(const char *filename, HWSize_t *size) {
  ...
  // Halt the program if buf has a memory error
  Verify333(buf != NULL);
  ...
  return NULL;
      ...
      // There are result more bytes read
      numread += result;
      // There are result less bytes to read
      left_to_read -= result;
    }
  }
...
static void AddToHashtable(HashTable tab, char *word, DocPositionOffset_t pos) {
  ...
  if (retval == 1) {
    ...
    // Halt program if out of memory error with wp
    Verify333(wp != NULL);
    // Allocate space for string and delimiter
    newstr = (char *) malloc(strlen(word) + 1);
    // Halt program if out of memory error with newstr
    Verify333(newstr != NULL);
    strncpy(newstr, word, strlen(word) +1);
    wp->word = newstr;
    wp->positions = AllocateLinkedList();
    // Halt program if out of memory error with positions
    Verify333(wp->positions != NULL);
    retval = AppendLinkedList(wp->positions, (LLPayload_t) ((intptr_t) pos));
    Verify333(retval != 0);
    kv.key = hashKey;
    kv.value = (void *) wp;
    retval = InsertHashTable(tab, kv, &oldkeyvalue);
    Verify333(retval == 1);
  }
}
