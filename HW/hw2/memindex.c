...
int MIAddPostingList(MemIndex index, char *word, DocID_t docid,
  ...
  if (res == 0) {
    ...
    // Halt program if out of memory error with WordDocSet
    Verify333(wds != NULL);
    wds->word = word;
    wds->docIDs = AllocateHashTable(1024);
    // Halt program if out of memory error with the hashtable
...
LinkedList MIProcessQuery(MemIndex index, char *query[], uint8_t qlen) {
  ...
  if (res == 0) {
    ...
    wds = (WordDocSet *) kv.value;
    HTIter it = HashTableMakeIterator(wds->docIDs);
    Verify333(it != NULL);
    while (!HTIteratorPastEnd(it)) {
      Verify333(HTIteratorGet(it, &kv) == 1);
      // Allocate and initialize SearchResult
      SearchResult *searchresult = \
      (SearchResult *) malloc(sizeof(SearchResult));
      Verify333(searchresult != NULL);
      searchresult->docid = kv.key;
      searchresult->rank = NumElementsInLinkedList(kv.value);
      Verify333(AppendLinkedList(retlist, searchresult));
      HTIteratorNext(it);
    }
    HTIteratorFree(it);  // Free iterator after use
  }
  ...
  for (i = 1; i < qlen; i++) {
    ...
    for (j = 0; j < ne; j++) {
      ...
      // Check if the docIDs have a match
      res = LookupHashTable(wds->docIDs, searchresult->docid, &kv);
      Verify333(res != -1);
      if (res == 1) {
        searchresult->rank += NumElementsInLinkedList(kv.value);
        LLIteratorNext(llit);
