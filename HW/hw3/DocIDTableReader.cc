...
bool DocIDTableReader::LookupDocID(const DocID_t &docid,
  ...
  for (auto it = elements.begin(); it != elements.end(); it++) {
    ...
    Verify333(fread(&header.docid, sizeof(header.docid), 1, file_) == 1);
    header.toHostFormat();
    // Is it a match?
    if (header.docid == docid) {
      ...
      // MISSING:
      Verify333(fread(&header.num_positions, \
        sizeof(header.num_positions), 1, file_) == 1);
      header.toHostFormat();
...
list<docid_element_header> DocIDTableReader::GetDocIDList() {
  ...
  for (HWSize_t i = 0; i < header_.num_buckets; i++) {
    ...
    // the index file .
    // MISSING:
    Verify333(fseek(file_, offset_ + 4 + i * 8, SEEK_SET) == 0);

    // Read in the chain length and bucket position fields from
    // the bucket_rec.
    bucket_rec b_rec;
    // MISSING:
    Verify333(fread(&b_rec.chain_len, sizeof(b_rec.chain_len), 1, file_) == 1);
    Verify333(fread(&b_rec.bucket_position, \
      sizeof(b_rec.bucket_position), 1, file_) == 1);
    b_rec.toHostFormat();
    ...
    for (HWSize_t j = 0; j < b_rec.chain_len; j++) {
      ...
      // MISSING:
      Verify333(fread(&ep.element_position, \
        sizeof(ep.element_position), 1, file_) == 1);
      ep.toHostFormat();
