...
DocIDTableReader *IndexTableReader::LookupWord(const std::string &word) {
  ...
  for (auto it = elements.begin(); it != elements.end(); it++) {
    ...
    Verify333(fread(&header.word_len, sizeof(header.word_len), 1, file_) == 1);
    Verify333(fread(&header.doctable_len,
      sizeof(header.doctable_len), 1, file_) == 1);
    header.toHostFormat();
    ...
    for (int i = 0; i < header.word_len; i++) {
      // MISSING:
      uint8_t nextc;
      Verify333(fread(&nextc, sizeof(nextc), 1, file_) == 1);
      ss << nextc;
    }
