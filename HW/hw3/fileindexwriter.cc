...
HWSize_t WriteIndex(MemIndex mi, DocTable dt, const char *filename) {
  ...
  // MISSING:
  HWSize_t headres = WriteHeader(f, dtres, mires);
  if (headres == 0) {
    fclose(f);
    unlink(filename);
    return 0;
...
static HWSize_t WriteDocPositionListFn(FILE *f,
  ...
  if (res != 0)
    return 0;
  DocID_t docID_disk = htonll(docID_ho);
  HWSize_t num_pos_disk = htonl(num_pos_ho);
  if (fwrite(&docID_disk, sizeof(docID_disk), 1, f) != 1)
    return 0;
  if (fwrite(&num_pos_disk, sizeof(num_pos_disk), 1, f) != 1)
    return 0;
  ...
  return sizeof(docID_ho) + sizeof(num_pos_ho)  + num_pos_ho * sizeof(position);
}
...
static HWSize_t WriteWordDocSetFn(FILE *f,
  ...
  HWSize_t htlen_ho = WriteHashTable(f,
                                     wds->docIDs,
                                     offset + \
                                     sizeof(worddocset_header) + wordlen_ho,
...
static HWSize_t WriteBucket(FILE *f,
  ...
  if (chainlen_ho > 0) {
    ...
    for (j = 0; j < chainlen_ho; j++) {
      ...
      // MISSING:
      HWSize_t nextelpos_no = htonl(nextelpos);
      ...
      // Write at the position
      if (fwrite(&nextelpos_no, sizeof(nextelpos_no), 1, f) != 1) {
        LLIteratorFree(it);
        return 0;
      }
      ...
      // Using "fn" parameter to write out the payload into the element
...
static HWSize_t WriteHashTable(FILE *f,
  ...
  for (i = 0; i < ht->num_buckets; i++) {
    ...
    HWSize_t chainlen_ho = NumElementsInLinkedList(list);
    res = WriteBucketRecord(f,
      list,
      next_bucket_rec_offset, next_bucket_offset);
    if (res == 0)
        return 0;
    next_bucket_rec_offset += res;

    if (chainlen_ho != 0) {
      res = WriteBucket(f, list, next_bucket_offset, fn);
      if (res == 0)
        return 0;
