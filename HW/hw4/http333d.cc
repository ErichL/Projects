...
void GetPortAndPath(int argc,
  ...
  // Sane number of command line arguments
  if (argc < 4)
    Usage(argv[0]);
  ...
  *port = atoi(argv[1]);
  if (*port < 1024 || *port > 65536) {
    cerr << "The port number was not reasonable" << endl;
    Usage(argv[0]);
  }
  ...
  if (stat(argv[2], &dir) == -1) {
    cerr << argv[2] << " is not a readable directory." << endl;
    Usage(argv[0]);
  }
  ...
  *path = std::string(argv[2]);
  ...
  for (int i = 3; i < argc; i++) {
    ...
    if (index.length() >= 4 && index.substr(index.length() - 4) == ".idx") {
      struct stat fstat;
      // Check that the index is readable
      ...
      if (!S_ISREG(fstat.st_mode)) {
        cerr << argv[i] << " is not a file." << endl;
        Usage(argv[0]);
      }
  ...
  // If no indexes were readable or were files
  if (indices->size() == 0) {
    cerr << "No indexes were readable files." << endl;
    Usage(argv[0]);
  }
}
